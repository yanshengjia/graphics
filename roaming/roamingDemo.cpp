...
// 各种flag
bool flagView = false;	// 按下左键标志
bool flagFirstPerson = false;	// 第一人称漫游模式标志
bool flagThirdPerson = false;	// 第三人称漫游模式标志
bool flagFly = false;	//飞机飞翔标志
...
void display()
{
	...
	if(flagFirstPerson)
	{
		...
		gluLookAt(human_x, human_y, human_z, human_x - 5 * cos((double)vangle / 180.0 * PI) , BASIC_SIZE * (BODY_SCALE_Y + THIGH_SCALE_X) / 2, human_z + 5 * sin((double)vangle / 180.0 * PI), 0.0f, 1.0f, 0.0f);	// 转换观察点
	}
...
void menu(int id)
{
    switch(id)
    {
		case 1:
			//第一人称漫游
			flagFirstPerson = true;
		case 2:
			//第三人称漫游
			flagThirdPerson = true;
		case 3:
			//飞机漫游
			flagFly = true;
        case 4:
...
int main(int argc, char *argv[])
{
	...
	// 设置右键弹出式菜单
    glutCreateMenu(menu);
	glutAddMenuEntry("first person roaming", 1);
	glutAddMenuEntry("third person roaming", 2);
	glutAddMenuEntry("fly",3);
    glutAddMenuEntry("aircraft revoke", 4);
